...
# __all__ = ["AutoAugmentPolicy", "AutoAugment", "Compose", "ToTensor", "Resize", "RandomHorizontalFlip", "CenterCrop", "Normalize"]
...
class RandomRotation:
    def __init__(self):
        self.rand_angle = np.rad2deg(random.random() * np.pi)
    def __call__(self, image, target):
        image = F.rotate(image, self.rand_angle, fill=[0,])
        target = F.rotate(target, self.rand_angle, fill=[0,])
        return image, target
...
class ShearX:
    ...
    def __call__(self, image, target):
        image = F.affine(
            ...
            translate=[0, 0],
            scale=1.0,
            shear=[math.degrees(math.atan(self.magnitude)), 0.0],
            fill=[0,]
        )
        target = F.affine(
            target,
...
class ShearY:
    ...
    def __call__(self, image, target):
        image = F.affine(
            ...
            translate=[0, 0],
            scale=1.0,
            shear=[0.0, math.degrees(math.atan(self.magnitude))],
            fill=[0,]
        )
        target = F.affine(
            target,
...
# rotation, shearing, not translation, no scaling, horizontal flips and vertical flips, no crop 
...
#                 mask = _apply_op(mask, op_name, magnitude, interpolation=self.interpolation, fill=fill)
